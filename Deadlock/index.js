...
function BuildFormFields($amount) // For Resorce allocation
{
  ...
  for ($i = 0; $i < $amount; $i++) {
    ...
    $field.innerHTML = 'Total Resources of res:' + $i;
...
function BuildFormFields2($amount) //For Allocation
{
  ...
  for ($i = 0; $i < $amount; $i++) {
    for ($j = 0; $j < res; $j++) {
      ...
      $item.style.margin = '10px';
      $field = document.createElement('label');
      $field.innerHTML = 'Resource Need for the process ' + $i + ' Res: ' + $j;
      $item.appendChild($field);
      $field = document.createElement('input');
      $field.name = 'Design[' + $i + ']';
      $field.type = 'text';
...
function BuildFormFields3($amount) //For Allocation
{
  ...
  for ($i = 0; $i < $amount; $i++) {
    for ($j = 0; $j < res; $j++) {
      ...
      $item.style.margin = '10px';
      $field = document.createElement('label');
      $field.innerHTML = 'Resources allocated to the process ' + $i + ' Res: ' + $j;
      $item.appendChild($field);
      $field = document.createElement('input');
      $field.name = 'Design[' + $i + ']';
      $field.type = 'text';
